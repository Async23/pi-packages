...
const NOTIFIER_APP =
  process.env.PI_NOTIFY_APP || join(HOME, "Applications", "Pi Notifier.app");
...
export function notificationSummary(value: unknown): string {
  const lines = String(value ?? "")
    ...
    .filter(Boolean);
  while (lines.length > 1 && GENERIC_HEADINGS.has(lines[0])) lines.shift();
  const first = lines[0] || "当前任务已执行完成。";
...
export function safeId(value: unknown): string {
  return compactText(value).replace(/[^A-Za-z0-9_.-]/gu, "_").slice(0, 96) || "unknown";
}
...
async function readTmuxTarget(pi: ExtensionAPI): Promise<TmuxTarget | undefined> {
  ...
  const tmux = (await isExecutable("/opt/homebrew/bin/tmux"))
...
async function deliverNotification(
  ...
  const [target, project] = await Promise.all([
    readTmuxTarget(pi),
    projectTitle(pi, ctx.cwd),
  ]);
  const threadTitle = shortDisplayText(
    pi.getSessionName() || cleanPaneTitle(target?.paneTitle) || project,
    THREAD_TITLE_MAX_WIDTH,
  );
  const title = target?.coordinate
    ? `${target.coordinate} · ${threadTitle || project}`
    : threadTitle || project;
  const subtitle = notificationSubtitle(firstNonEmptyLine(prompt)) || "任务完成";
  ...
  for (const notifier of candidates) {
    ...
    try {
      const result = await pi.exec(
        notifier,
        buildNotifierArgs(content, iconPath, focusCommand),
        { timeout: NOTIFIER_TIMEOUT_MS },
      );
      if (result.code === 0) {
        success = true;
...
export default function piNotifyExtension(pi: ExtensionAPI): void {
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    if (notificationsDisabled() || ctx.mode !== "tui" || !ctx.isIdle()) return;
  ...
  pi.registerCommand("pi-notify-setup", {
    description: "Install the dedicated macOS Pi Notifier app",
    handler: async (args, ctx) => {
      if (args.trim() !== "") {
        ctx.ui.notify("Usage: /pi-notify-setup", "error");
        return;
      }
      ...
      try {
        ...
        ctx.ui.notify("Pi Notifier app installed. Run /pi-notify-test.", "info");
      } catch (error) {
  ...
  pi.registerCommand("pi-notify-test", {
    ...
    handler: async (_args, ctx) => {
      ...
      const result = await deliverNotification(
        ...
        ctx,
        "Pi 通知测试",
        "通知已启用；点击此通知应返回当前 Ghostty/tmux pane。",
      );
      ctx.ui.notify(
        result.success
