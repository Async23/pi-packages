...
Connecting is deliberately explicit in this version: opening `/mcp` never starts commands or contacts remote endpoints. After a connection succeeds, each MCP tool is registered in Pi as its normalized remote name plus a stable eight-character identity suffix, for example `list_pages__kuX-ij0s`. The suffix derives from the runtime instance identity and remote tool name, so reconnecting preserves the Pi Tool Name while same-name tools from different MCP instances remain distinct. Resource access uses the same compact form, such as `read_resource__Ab12cd34`. The bridge guards its own generated name mapping against internal collisions, while Pi handles collisions with Tools registered elsewhere. Prompts remain user-controlled and are invoked with `p` from the panel rather than exposed as model-controlled tools.
